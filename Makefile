...
# The management cluster name must match the one in test/e2e/config/freebox.yaml
E2E_MANAGEMENT_CLUSTER ?= freebox-test
...
cleanup-test-e2e: ## Tear down the Kind cluster used for e2e tests
	@$(KIND) delete cluster --name $(E2E_MANAGEMENT_CLUSTER) 2>/dev/null || true
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name cluster-api-provider-freebox-builder
	$(CONTAINER_TOOL) buildx use cluster-api-provider-freebox-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm cluster-api-provider-freebox-builder
	rm Dockerfile.cross
